...
class MaharashtraAgriculturalSystem:
    def __init__(self):
        ...
        self.fertilizer_data = {
            ...
            'NPK 19:19:19': {'price': 22.00, 'n_content': 19, 'p_content': 19, 'k_content': 19},
            'Single Super Phosphate': {'price': 12.00, 'n_content': 0, 'p_content': 16, 'k_content': 0},
            'Potash': {'price': 18.00, 'n_content': 0, 'p_content': 0, 'k_content': 50},
            'Zinc Sulphate': {'price': 45.00, 'n_content': 0, 'p_content': 0, 'k_content': 0},
            'Organic Compost': {'price': 8.50, 'n_content': 2, 'p_content': 1, 'k_content': 1}
        }
    ...
    def generate_weather_trends(self, district, days=7):
        ...
        temperatures = np.clip(temp_base + temp_var * (np.random.random(days) - 0.5), 10, 45)
        humidity = np.clip(humidity_base + humidity_var * (np.random.random(days) - 0.5), 20, 95)
    ...
    def generate_weather_forecast(self, district, days=5):
        ...
        conditions = np.random.choice(['sunny', 'partly cloudy', 'cloudy', 'light rain', 'moderate rain'], days)
        ...
        try:
            if uploaded_file is not None:
                ...
                if uploaded_file.type not in ['image/jpeg', 'image/jpg', 'image/png']:
                    return {'error': 'Please upload a valid image file (JPEG, JPG, or PNG)'}
                
                if uploaded_file.size > 10 * 1024 * 1024:  # 10MB limit
                    return {'error': 'File size too large. Please upload an image smaller than 10MB.'}
                ...
                try:
                    ...
                    image_array = np.array(image.resize((224, 224), Image.Resampling.LANCZOS)).astype(np.float32) / 255.0
                except AttributeError:
                    # Fallback for older PIL versions
                    image_array = np.array(image.resize((224, 224), Image.LANCZOS)).astype(np.float32) / 255.0
                ...
                if quality_score < 0.3:
                    return {
                        ...
                        'recommendations': ['Take image in natural daylight', 'Ensure leaf fills most of frame', 'Avoid shadows and reflections']
                    }
                ...
                if self.disease_model is not None:
                    try:
                        ...
                        final_predictions = np.average(all_predictions, axis=0, weights=confidence_weights)
                        ...
                        if adjusted_confidence < 0.5:
                            disease = "Uncertain Analysis - Expert Consultation Recommended"
                        ...
                        all_disease_predictions = [(self.class_names[i], float(final_predictions[i]) * 100 * (0.7 + 0.3 * quality_score)) 
                        ...
                        return {
                            ...
                            'severity_assessment': self.assess_disease_severity(adjusted_confidence * 100, disease)
                        }
                    except Exception as model_error:
                        st.warning(f"AI Model unavailable. Using advanced image analysis: {model_error}")
    ...
    def advanced_image_quality_assessment(self, image_array):
        ...
        quality_score = (brightness_score * 0.25 + contrast_score * 0.25 + 
                        sharpness_score * 0.25 + balance_score * 0.15 + noise_score * 0.1)
    ...
    def extract_advanced_image_features(self, image_array):
        ...
        dark_spots = np.sum((hsv_approx[:,:,2] < 0.3) & (hsv_approx[:,:,1] > 0.2)) / image_array.size
        brown_spots = np.sum((hsv_approx[:,:,0] > 0.08) & (hsv_approx[:,:,0] < 0.17) & 
                           (hsv_approx[:,:,1] > 0.3)) / image_array.size
        
        # Yellow/chlorotic areas
        yellow_areas = np.sum((hsv_approx[:,:,0] > 0.15) & (hsv_approx[:,:,0] < 0.18) & 
                            (hsv_approx[:,:,1] > 0.5)) / image_array.size
    ...
    def rgb_to_hsv_approx(self, rgb_array):
        ...
        hue = np.where(diff == 0, 0,
              np.where(max_val == r, (60 * ((g - b) / diff) + 360) % 360,
              np.where(max_val == g, (60 * ((b - r) / diff) + 120) % 360,
                      (60 * ((r - g) / diff) + 240) % 360)))
    ...
    def advanced_disease_simulation(self, features, quality_score):
        ...
        if (green_dom > 0.4 and dark_spots < 0.05 and brown_spots < 0.03 and 
            yellow_areas < 0.02 and brightness > 0.3 and texture_var < 0.02):
            confidence = np.random.uniform(88, 96) * quality_multiplier
            return "Healthy", min(95, confidence)
        
        # Early blight detection
        elif (brown_spots > 0.08 and dark_spots > 0.05 and edge_density > 0.3 and 
              texture_var > 0.03):
    ...
    def get_error_response(self):
        ...
        return {
            ...
            'recommendations': ['Please upload a clear image', 'Ensure good lighting conditions'],
    ...
    def assess_image_quality(self, image_array):
        ...
        if brightness > 0.1 and brightness < 0.9 and contrast > 0.05 and sharpness > 0.01:
            return "Good - High accuracy expected"
    ...
    def simulate_disease_detection(self, features):
        ...
        if (green_dom > 1.3 and dark_spots < 0.08 and brightness > 0.45 and 
            color_uniformity > 0.8 and texture_var < 0.03):
    ...
    def generate_realistic_predictions(self, detected_disease, confidence):
        ...
        detected_normalized = detected_disease.replace('___', ' ').replace('_', ' ').replace('  ', ' ')
        
        # Check if detected disease indicates healthy plant
        is_healthy = any(h in detected_normalized.lower() for h in ['healthy', 'tomato healthy'])
        if is_healthy:
        ...
        for disease in diseases:
            ...
            if disease_normalized == detected_lower or \
               (is_healthy and 'healthy' in disease_normalized):
                ...
                elif 'bacterial' in detected_lower and 'bacterial' in disease_normalized:
        ...
        if total > 0:
            predictions = [(pred[0], round(pred[1] * 100 / total, 1)) for pred in predictions]
    ...
    def calculate_ndvi(self, nir_value, red_value):
        ...
        try:
            # Input validation
            if not isinstance(nir_value, (int, float)) or not isinstance(red_value, (int, float)):
                return 0
    ...
    def analyze_soil_health(self, ph, nitrogen, phosphorus, potassium, farm_area=1.0):
        ...
        if ph < optimal_ph_range[0]:
            acidity_level = "moderately acidic" if ph > 5.5 else "strongly acidic" if ph > 5.0 else "very strongly acidic"
            recommendations.append(f"Soil is {acidity_level} (pH {ph}). Lime application required.")
            ...
            alkalinity_level = "moderately alkaline" if ph < 8.5 else "strongly alkaline"
            recommendations.append(f"Soil is {alkalinity_level} (pH {ph}). Organic matter and sulfur needed.")
            ...
            fertilizer_recommendations.extend([
                {
                    ...
                    'cost': round(organic_matter_needed * self.fertilizer_data['Organic Compost']['price'], 2),
        ...
        if n_deficit > 0:
            ...
            urea_needed = (n_deficit * farm_area) / (self.fertilizer_data['Urea']['n_content'] / 100) / urea_efficiency
            
            severity = "severe" if n_deficit > 100 else "moderate" if n_deficit > 50 else "mild"
            recommendations.append(f"Nitrogen deficiency: {severity} ({n_deficit} kg/ha deficit). Split application recommended.")
            
            fertilizer_recommendations.append({
                'type': 'Urea (46% N)',
                'quantity': round(urea_needed, 1),
                'cost': round(urea_needed * self.fertilizer_data['Urea']['price'], 2),
        ...
        if p_deficit > 0:
            ...
            dap_needed = (p_deficit * farm_area * p_fixation_factor) / (self.fertilizer_data['DAP']['p_content'] / 100)
            
            severity = "severe" if p_deficit > 8 else "moderate" if p_deficit > 4 else "mild"
            recommendations.append(f"Phosphorus deficiency: {severity} ({p_deficit} kg/ha deficit). Band placement recommended.")
        ...
        if k_deficit > 0:
            ...
            mop_needed = (k_deficit * farm_area) / (self.fertilizer_data['MOP']['k_content'] / 100) / k_availability
            
            severity = "severe" if k_deficit > 75 else "moderate" if k_deficit > 40 else "mild"
            recommendations.append(f"Potassium deficiency: {severity} ({k_deficit} kg/ha deficit). Split application needed.")
        ...
        if n_deficit == 0 and p_deficit == 0 and k_deficit == 0:
            recommendations.append("Excellent nutrient status! Maintenance fertilization recommended.")
            maintenance_npk = 75 * farm_area  # kg/ha for maintenance
            
            fertilizer_recommendations.append({
                'type': 'NPK 19:19:19',
                'quantity': round(maintenance_npk, 1),
                'cost': round(maintenance_npk * self.fertilizer_data['NPK 19:19:19']['price'], 2),
        ...
        secondary_nutrients = self.analyze_secondary_micronutrients(ph, nitrogen, phosphorus, potassium)
        ...
        return {
            ...
            'soil_classification': self.classify_soil_type(ph, nitrogen, phosphorus, potassium)
        }
    ...
    def analyze_secondary_micronutrients(self, ph, n, p, k):
        ...
        if n > 250 and ph > 7.0:  # High N uptake areas need more S
            recommendations.append("Sulfur deficiency likely. Apply gypsum or elemental sulfur.")
            fertilizers.append({
        ...
        if ph > 7.5 or (p > 12 and k < 100):  # High P can induce Zn deficiency
            recommendations.append("Zinc deficiency possible in alkaline soil. Apply zinc sulfate.")
            fertilizers.append({
        ...
        if p > 10 and k > 120:  # Good fertility soils may need B
            recommendations.append("Consider boron application for cotton/sunflower crops.")
            fertilizers.append({
    ...
    def analyze_pest_risk(self, weather_data, crop_type, growth_stage):
        ...
        rainfall_risk = self.calculate_rainfall_risk(np.mean(historical_weather['rainfall'][-7:]))
        ...
        pest_predictions = self.predict_specific_pests(crop_type, overall_risk, current_weather)
        
        return {
            'overall_risk': round(overall_risk, 1),
            'risk_factors': {
                'temperature': {'value': temp_risk, 'status': self.get_risk_status(temp_risk)},
                'humidity': {'value': humidity_risk, 'status': self.get_risk_status(humidity_risk)},
                'rainfall': {'value': rainfall_risk, 'status': self.get_risk_status(rainfall_risk)},
                'seasonal': {'value': seasonal_risk, 'status': self.get_risk_status(seasonal_risk)}
            },
            'crop_specific_risks': crop_risks,
            'pest_predictions': pest_predictions,
            'recommendations': self.generate_pest_recommendations(overall_risk, crop_type),
    ...
    def get_crop_specific_risks(self, crop_type, growth_stage):
        ...
        if crop_type in crop_pest_db:
            ...
            risk_multiplier = 1.5 if growth_stage in crop_data['high_risk_stages'] else 1.0
            return {
        ...
        return {'pests': [], 'diseases': [], 'risk_multiplier': 1.0, 'high_risk_stage': False}
    ...
    def predict_specific_pests(self, crop_type, overall_risk, weather):
        ...
        for pest in crop_risks['pests']:
            ...
            predictions.append({
                ...
                'severity': 'High' if final_probability > 70 else 'Medium' if final_probability > 40 else 'Low',
    ...
    def generate_pest_recommendations(self, risk_score, crop_type):
        ...
        if risk_score > 70:
            recommendations.extend([
                "🚨 **CRITICAL PEST ALERT**: High pest risk detected (Risk Score: {:.1f}/100). Immediate intervention required to prevent crop damage.".format(risk_score),
            ...
            recommendations.extend([
                "⚠️ **MODERATE PEST RISK**: Risk Score {:.1f}/100 indicates potential pest buildup. Take preventive action now.".format(risk_score),
            ...
            recommendations.extend([
                "✅ **LOW PEST RISK**: Current conditions show low pest pressure (Risk Score: {:.1f}/100). Maintain preventive measures.".format(risk_score),
    ...
    def get_irrigation_recommendations(self, crop_type, district, growth_stage, soil_ph, farm_area, current_weather):
        ...
        crop_water_needs = {
            'Cotton': {'min': 600, 'max': 1200, 'critical_stages': ['Flowering', 'Fruit Development']},
            'Rice': {'min': 1000, 'max': 1800, 'critical_stages': ['Vegetative', 'Flowering']},
            'Wheat': {'min': 400, 'max': 650, 'critical_stages': ['Tillering', 'Grain filling']},
            'Sugarcane': {'min': 1200, 'max': 2000, 'critical_stages': ['Grand growth', 'Maturity']},
            'Soybean': {'min': 450, 'max': 700, 'critical_stages': ['Flowering', 'Pod filling']},
            'Tomato': {'min': 500, 'max': 800, 'critical_stages': ['Flowering', 'Fruit Development']},
            'Potato': {'min': 400, 'max': 600, 'critical_stages': ['Tuber formation', 'Tuber bulking']},
            'Onion': {'min': 350, 'max': 550, 'critical_stages': ['Bulb formation', 'Maturity']},
            'Maize': {'min': 500, 'max': 800, 'critical_stages': ['Tasseling', 'Grain filling']},
            'Jowar': {'min': 300, 'max': 500, 'critical_stages': ['Panicle emergence', 'Grain filling']}
        }
        
        zone_data = zone_irrigation_data.get(zone, zone_irrigation_data['Western Maharashtra'])
        crop_data = crop_water_needs.get(crop_type, crop_water_needs['Cotton'])
        
        # Calculate water requirements
        base_requirement = crop_data['min'] + (crop_data['max'] - crop_data['min']) * 0.7  # 70% of max
        zone_adjustment = zone_data['base_water_need'] / 500  # Normalize to Western Maharashtra
        ...
        final_daily_need = adjusted_daily_need * temp_factor * humidity_factor * wind_factor
        
        # Calculate irrigation schedule
        irrigation_frequency = 'Daily' if final_daily_need > 8 else 'Every 2 days' if final_daily_need > 4 else 'Every 3 days'
        water_per_application = final_daily_need * farm_area * 10  # Convert to liters per hectare
        ...
        recommendations = {
            ...
            'recommendations': self.generate_irrigation_recommendations(zone, crop_type, growth_stage, final_daily_need)
        }
        
        return recommendations
    
    def generate_irrigation_recommendations(self, zone, crop_type, growth_stage, daily_need):
        """Generate specific irrigation recommendations"""
        recommendations = []
        
        # Base recommendations with detailed explanations
        if daily_need > 8:
            recommendations.append("⚠️ **HIGH WATER DEMAND**: Daily requirement {:.1f}mm indicates high evapotranspiration. Consider drip irrigation for 40-60% water savings.".format(daily_need))
            recommendations.append("🌅 **Optimal Timing**: Irrigate during early morning (5:30-8:00 AM) when evaporation rates are lowest. Avoid midday irrigation.")
            recommendations.append("📊 **Split Application**: Divide daily water into 2-3 smaller applications to improve soil water retention.")
        elif daily_need < 3:
            recommendations.append("✅ LOW water requirement. Monitor soil moisture before irrigating.")
            recommendations.append("📅 Extend irrigation intervals to prevent waterlogging.")
        else:
            recommendations.append("✅ MODERATE water requirement. Maintain regular irrigation schedule.")
        
        # Zone-specific recommendations
        if 'Coastal' in zone:
            recommendations.append("🌊 Coastal zone: Use rainwater harvesting during monsoon.")
            recommendations.append("🧂 Monitor soil salinity near coastal areas.")
        elif 'Marathwada' in zone:
            recommendations.append("💧 Marathwada zone: Focus on water conservation techniques.")
            recommendations.append("🌧️ Implement watershed management practices.")
        elif 'Vidarbha' in zone:
            recommendations.append("🏞️ Vidarbha zone: Utilize river water sources when available.")
            recommendations.append("💨 Consider wind breaks to reduce evaporation.")
        
        # Crop-specific recommendations
        if crop_type == 'Cotton':
            recommendations.append("🌱 Cotton: Avoid overhead irrigation during flowering to prevent pest issues.")
        elif crop_type == 'Rice':
            recommendations.append("🌾 Rice: Maintain 2-5 cm water depth in fields during vegetative stage.")
        elif crop_type == 'Sugarcane':
            recommendations.append("🎋 Sugarcane: Ensure adequate moisture during grand growth phase.")
        
        # Growth stage recommendations
        if growth_stage in ['Flowering', 'Fruit Development']:
            recommendations.append("🌸 Critical growth stage: Maintain consistent soil moisture.")
    ...
    def save_analysis_data(self, data):
        ...
        try:
            analysis_doc = {
                'timestamp': data[0] if isinstance(data[0], datetime) else datetime.now(),
                'district': str(data[1]),
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        with tab_login:
            st.markdown('<div class="form-header"><h2>Welcome Back! 👋</h2></div>', unsafe_allow_html=True)
            with st.form("login_form_main"):
                username = st.text_input("👤 Username or Email", placeholder="Enter your username or email")
                password = st.text_input("🔒 Password", 
                                       type="password" if not st.session_state.password_visible else "text",
                ...
                with col2:
                    st.markdown('<div style="text-align: right;"><a href="#" style="color: var(--secondary-green);">Forgot Password?</a></div>', 
                              unsafe_allow_html=True)
            ...
            if submit:
                if not username or not password:
                    ...
                    with st.spinner("🔄 Authenticating..."):
                        res = st.session_state.auth_db.authenticate_farmer(username, password, ip_address="127.0.0.1")
                        if res.get("success"):
        ...
        with tab_register:
            st.markdown('<div class="form-header"><h2>Create New Account 🌱</h2></div>', unsafe_allow_html=True)
            with st.form("register_form_main"):
                col1, col2 = st.columns(2)
                with col1:
                    full_name = st.text_input("👤 Full Name", placeholder="Enter your full name")
                    username_r = st.text_input("👤 Username", placeholder="Choose a username")
                with col2:
            ...
            if submit_r:
                if not all([full_name, username_r, email, password_r, password_confirm]):
    ...
    with st.sidebar:
        ...
        if selection_method == "By Zone":
            zone = st.selectbox("Select Zone", list(system.maharashtra_districts.keys()))
            district = st.selectbox("Select District", system.maharashtra_districts[zone])
        else:
        ...
        farm_area = st.number_input("Farm Area (Hectares)", min_value=0.1, max_value=1000.0, value=1.0, step=0.1)
        ...
        if st.button(
            ...
            # 2. Soil analysis
            soil_result = system.analyze_soil_health(soil_ph, nitrogen, phosphorus, potassium, farm_area)
            analyses['soil'] = soil_result
            st.session_state.soil_analysis = soil_result
            
            # 3. Pest risk analysis
            pest_result = system.analyze_pest_risk(weather_data, crop_type, growth_stage)
            analyses['pest'] = pest_result
            st.session_state.pest_analysis = pest_result
            
            # 4. Irrigation analysis
            irrigation_result = system.get_irrigation_recommendations(crop_type, district, growth_stage, soil_ph, farm_area, current_weather)
            ...
            disease_detected = analyses.get('crop', {}).get('disease', 'No image uploaded')
            confidence = analyses.get('crop', {}).get('confidence', 0)
            ndvi_val = system.calculate_ndvi(0.8, 0.3)  # Default NDVI
            
            analysis_data = (
                timestamp, district, crop_type, growth_stage, farm_area,
                disease_detected, confidence, ndvi_val, soil_ph, nitrogen,
                phosphorus, potassium, "Comprehensive analysis completed"
            )
            ...
            if 'crop' in analyses:
                st.write(f"🌿 Crop: {analyses['crop']['disease']} ({analyses['crop']['confidence']:.1f}% confidence)")
            st.write(f"🧪 Soil Health: {analyses['soil']['score']}/100 ({analyses['soil']['status']})")
            st.write(f"🐛 Pest Risk: {analyses['pest']['overall_risk']}/100 ({analyses['pest']['risk_level']['level']})")
            st.write(f"💧 Irrigation: {analyses['irrigation']['daily_water_requirement']:.1f} mm/day ({analyses['irrigation']['irrigation_frequency']})")
            st.write(f"💰 Fertilizer Cost: ₹{analyses['soil']['total_cost']:.2f}")

        # Download PDF report button (adds only one button)
        try:
            pdf_buf = generate_pdf_report(district, zone, crop_type, growth_stage, farm_area, current_weather)
            st.download_button(
    ...
    tab_names = ["🌱 Crop Health", "🌤️ Weather & Soil", "🐛 Pest Risk", "💧 Irrigation", "🗺️ Zone Mapping", "📊 Dashboard"]
    tabs = st.tabs(tab_names)
    
    # Enhanced Crop Health Tab
    with tabs[0]:
        st.markdown('<div class="tab-header"><h2>🌿 Advanced Crop Health Monitoring & Analysis</h2></div>', unsafe_allow_html=True)
    ...
    ''', unsafe_allow_html=True)
                ...
                alert_color = "#F44336" if confidence > 80 else "#FF9800" if confidence > 60 else "#2196F3"
                st.markdown(f'''
                <div style="
        ...
        with col1:
            ...
            st.markdown('<div class="upload-area" style="border: 3px dashed #4CAF50; background: linear-gradient(135deg, rgba(76,175,80,0.1), rgba(102,187,106,0.1)); padding: 2rem; border-radius: 15px; text-align: center;">', unsafe_allow_html=True)
            uploaded_file = st.file_uploader(
            ...
            if uploaded_file:
                ...
                with col_info:
                    ...
                    quality_score = "Excellent" if brightness > 0.3 and brightness < 0.8 else "Good" if brightness > 0.2 and brightness < 0.9 else "Fair"
                    quality_color = "#4CAF50" if quality_score == "Excellent" else "#FF9800" if quality_score == "Good" else "#F44336"
                    
                    st.markdown(f'<div style="color: {quality_color}; font-weight: bold;">📈 Quality: {quality_score}</div>', unsafe_allow_html=True)
                ...
                if st.button("🔬 AI CROP HEALTH ANALYSIS", type="primary", key="analyze_crop_button", help="Advanced AI analysis using deep learning models"):
                ...
                st.info("💡 Upload a clear crop image for AI-powered disease detection and analysis.")
        ...
        with col2:
            ...
            st.markdown('<div class="ndvi-section" style="background: rgba(255,255,255,0.05); padding: 1.5rem; border-radius: 12px; border: 1px solid rgba(25,118,210,0.2);">', unsafe_allow_html=True)
            ...
            with col_nir:
                ...
                st.markdown('<p style="font-size: 12px; color: #B8D4B8;">💡 Healthy: 0.7-0.9</p>', unsafe_allow_html=True)
            ...
            with col_red:
                ...
                st.markdown('<p style="font-size: 12px; color: #B8D4B8;">💡 Healthy: 0.1-0.4</p>', unsafe_allow_html=True)
            ...
            ndvi_fig = go.Figure(go.Indicator(
                ...
                gauge={'axis': {'range': [-1, 1]},
                       ...
                       'steps': [
                           ...
                           {'range': [0.3, 0.5], 'color': "#FFA726"},   # Sunshine Yellow
                           {'range': [0.5, 0.7], 'color': "#66BB6A"},   # Light Green
                           {'range': [0.7, 1], 'color': "#4CAF50"}],    # Bright Green
            ...
            # NDVI Interpretation
            st.markdown(f'<div style="color: {ndvi_color}; font-weight: bold; text-align: center;">{ndvi_interpretation}</div>', 
                       unsafe_allow_html=True)
            ...
            with env_col1:
                season = st.selectbox("Season", ["Kharif", "Rabi", "Zaid"], help="Different seasons affect NDVI values")
                growth_stage_ndvi = st.selectbox("Growth Stage", ["Germination", "Vegetative", "Flowering", "Maturity"], key="ndvi_growth")
            
            with env_col2:
                water_status = st.selectbox("Water Status", ["Adequate", "Deficit", "Excess"], help="Water availability affects vegetation health")
                soil_type_ndvi = st.selectbox("Soil Type", ["Black Cotton", "Red Soil", "Alluvial", "Laterite"], key="ndvi_soil")
        ...
        if 'crop_analysis' in st.session_state and st.session_state.crop_analysis is not None:
            ...
            st.markdown("---")
            st.markdown('<div style="text-align: center; margin: 2rem 0;"><h2 style="color: #2E7D32;">🧬 AI Analysis Results & Treatment Plan</h2></div>', unsafe_allow_html=True)
            ...
            with col3:
                severity = "LOW" if result['confidence'] < 60 else "MEDIUM" if result['confidence'] < 80 else "HIGH"
                st.metric("Severity Level", severity)
            
            with col4:
                health_status = "Healthy" if result['disease'].lower() == 'healthy' else "Needs Attention"
            ...
            with col1:
                if 'all_predictions' in result:
                    ...
                    pred_df = pd.DataFrame(result['all_predictions'], columns=['Disease', 'Probability'])
                    
                    # Enhanced bar chart with healthy status highlighting
                    pred_fig = go.Figure()
                    
                    # Color code bars - green for healthy, blue for diseases
                    colors = ['#4CAF50' if 'healthy' in disease.lower() else '#3498DB' 
            ...
            with col2:
                ...
                risk_level = "LOW" if result['confidence'] < 60 else "MEDIUM" if result['confidence'] < 80 else "HIGH"
                ...
                quality_color = "#4CAF50" if quality == "Excellent" else "#FF9800" if quality == "Good" else "#F44336"
            ...
            st.markdown("---")
            st.markdown('<div style="text-align: center; margin: 2rem 0;"><h2 style="color: #2E7D32;">💊 Comprehensive Treatment Plan</h2></div>', unsafe_allow_html=True)
            ...
            if result['disease'].lower() == 'healthy':
                ...
                if protocol:
                    ...
                    tab1, tab2, tab3, tab4 = st.tabs(["🚨 Immediate Actions", "💊 Treatment Protocol", "🛡️ Prevention Measures", "📊 Monitoring Plan"])
                    ...
                    with tab1:
                        ...
                        if result['confidence'] > 80:
                            st.error("⚠️ HIGH CONFIDENCE DETECTION - Start treatment immediately!")
                    ...
                    with tab2:
                        ...
                        st.info(f"💵 **Approximate Cost**: {cost} (including labor and materials)")
                    ...
                    with tab3:
                        ...
                        st.success("🔄 Implement integrated pest management (IPM) practices for sustainable crop health.")
    ...
    # Weather & Soil Tab
    with tabs[1]:
        st.markdown('<div class="tab-header"><h2>Weather & Soil Analysis</h2></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            temp_fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(46, 125, 50, 0.2)')
            temp_fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(46, 125, 50, 0.2)')
            ...
            rain_fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(25, 118, 210, 0.2)')
            rain_fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(25, 118, 210, 0.2)')
        ...
        with col2:
            ...
            humidity_fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(76, 175, 80, 0.2)')
            humidity_fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(76, 175, 80, 0.2)')
            ...
            wind_fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(141, 110, 99, 0.2)')
            wind_fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(141, 110, 99, 0.2)')
        ...
        with col2:
            ...
            weather_fig.update_layout(
                ...
                yaxis2=dict(title='Humidity (%)', side='right', overlaying='y', color='#4CAF50'),
                height=400,
                plot_bgcolor='rgba(15, 20, 25, 0.02)',
                paper_bgcolor='rgba(15, 20, 25, 0.02)',
                font=dict(color='#E8F5E8')
        ...
        soil_analysis = system.analyze_soil_health(soil_ph, nitrogen, phosphorus, potassium, farm_area)
        
        # Enhanced soil metrics with better visualization
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            score_color = '#4CAF50' if soil_analysis['score'] > 80 else '#FF9800' if soil_analysis['score'] > 60 else '#F44336'
            st.markdown(f'<div style="background: linear-gradient(135deg, {score_color}, {score_color}AA); padding: 1.5rem; border-radius: 15px; text-align: center; color: white; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h2>{soil_analysis["score"]}/100</h2><p style="margin: 0; font-size: 1.1em;">Soil Health Score</p></div>', unsafe_allow_html=True)
        with col2:
            delta_color = 'normal' if abs(soil_ph - 6.8) < 0.5 else 'inverse'
            st.metric("pH Level", f"{soil_ph}", delta=f"{soil_ph - 6.8:.1f} from ideal", delta_color=delta_color)
        with col3:
        ...
        with col1:
            ...
            npk_fig.add_trace(go.Bar(
                name='Current Level',
                x=nutrients,
                y=current_values,
                marker_color='#ff4d4d',
                text=['Low' if curr < opt * 0.8 else 'High' if curr > opt * 1.2 else 'Good' 
        ...
        with col2:
            ...
            if 'fertilizer_costs' in soil_analysis:
                ...
                cost_fig.update_layout(
                    title='Fertilizer Cost Distribution',
                    height=300
                )
            ...
            ph_fig = go.Figure()
            ph_fig.add_trace(go.Bar(
                x=["Current pH"],
                y=[soil_ph],
                marker_color='#ff4d4d' if soil_ph < 6.0 or soil_ph > 7.5 else '#00cc96',
                text=[f"pH {soil_ph:.1f} ({'Acidic' if soil_ph < 6.0 else 'Alkaline' if soil_ph > 7.5 else 'Optimal'})"],
                textposition='auto'
            ))
            
            # Add optimal range indicators
            ph_fig.add_hline(y=6.0, line_dash="dash", line_color="green", 
                           annotation_text="Optimal Min (6.0)", annotation_position="right")
            ph_fig.add_hline(y=6.8, line_dash="dot", line_color="lime", 
                           annotation_text="Ideal (6.8)", annotation_position="right")
            ph_fig.add_hline(y=7.5, line_dash="dash", line_color="green", 
                           annotation_text="Optimal Max (7.5)", annotation_position="right")
        ...
        with col1:
            ...
            ph_status = 'Good' if 6.0 <= soil_ph <= 7.5 else ('Acidic' if soil_ph < 6.0 else 'Alkaline')
            ...
            ph_fig.add_hline(y=6.0, line_dash="dash", line_color="green", annotation_text="Optimal Min (6.0)")
            ph_fig.add_hline(y=7.5, line_dash="dash", line_color="green", annotation_text="Optimal Max (7.5)")
            ph_fig.add_hline(y=6.8, line_dash="dot", line_color="blue", annotation_text="Ideal (6.8)")
        ...
        with col2:
            ...
            if soil_analysis['fertilizer_recommendations']:
                ...
                fert_fig = go.Figure(data=[go.Pie(
                    labels=fert_df['type'],
                    values=fert_df['cost'],
                    hole=0.4,
                    marker=dict(
                        colors=['#3498DB', '#E67E22', '#9B59B6', '#F1C40F', '#E74C3C'],
                        line=dict(color='white', width=2)
                    ),
                    textinfo='label+percent',
                ...
                st.info("🌱 No specific fertilizer recommendations available. Soil health appears optimal!")
    ...
    # Pest Risk Tab
    with tabs[2]:
        st.markdown('<div class="tab-header"><h2>🐛 Pest Risk Assessment & Alert System</h2></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            categories = ['Temperature Risk', 'Humidity Risk', 'Rainfall Risk', 'Seasonal Risk']
            values = [risk_factors[cat]['value'] for cat in ['temperature', 'humidity', 'rainfall', 'seasonal']]
            ...
            if pest_analysis['pest_predictions']:
                ...
                for severity in ['Low', 'Medium', 'High']:
                    ...
                    if not filtered_df.empty:
                        color = '#44AA44' if severity == 'Low' else '#FFAA00' if severity == 'Medium' else '#FF4444'
                        pred_fig.add_trace(go.Bar(
                            ...
                            textposition='outside',
                            hovertemplate='<b>%{x}</b><br>Risk: %{y:.1f}%<br>Severity: ' + severity + '<extra></extra>'
                        ))
                ...
                pred_fig.add_hline(y=70, line_dash="dash", line_color="red", 
                                 annotation_text="High Risk Threshold")
                pred_fig.add_hline(y=30, line_dash="dash", line_color="orange", 
                                 annotation_text="Medium Risk Threshold")
                
                st.plotly_chart(pred_fig, use_container_width=True)
            else:
                st.info("🐛 No specific pest predictions available for current conditions")
        ...
        with col1:
            ...
            st.metric("Temperature Risk", f"{temp_risk['value']}/100", temp_risk['status'])
        
        with col2:
            humidity_risk = risk_factors['humidity']
            st.metric("Humidity Risk", f"{humidity_risk['value']}/100", humidity_risk['status'])
        
        with col3:
            rainfall_risk = risk_factors['rainfall']
            st.metric("Rainfall Risk", f"{rainfall_risk['value']}/100", rainfall_risk['status'])
        
        with col4:
            seasonal_risk = risk_factors['seasonal']
            st.metric("Seasonal Risk", f"{seasonal_risk['value']}/100", seasonal_risk['status'])
        ...
        if pest_analysis['pest_predictions']:
            for pred in pest_analysis['pest_predictions']:
                if pred['details'] and pred['probability'] > 30:  # Show details for significant risks
                    details = pred['details']
                    severity_color = '#FF4444' if pred['severity'] == 'High' else '#FFAA00' if pred['severity'] == 'Medium' else '#44AA44'
                    
                    with st.expander(f"🐛 {pred['pest']} - {pred['severity']} Risk ({pred['probability']}%)", expanded=(pred['severity'] == 'High')):
                        col1, col2 = st.columns([1, 1])
                        
                        with col1:
                            st.markdown(f"**🔬 Scientific Name:** *{details['scientific_name']}*")
                            st.markdown(f"**📅 Lifecycle:** {details['lifecycle_days']} days")
                            st.markdown(f"**🥚 Reproduction:** {details['eggs_per_female']} eggs/female")
                            
                            st.markdown("**🌡️ Favorable Conditions:**")
                            st.markdown(f"- Temperature: {details['favorable_conditions']['temperature'][0]}-{details['favorable_conditions']['temperature'][1]}°C")
                            st.markdown(f"- Humidity: {details['favorable_conditions']['humidity'][0]}-{details['favorable_conditions']['humidity'][1]}%")
                            st.markdown(f"- Rainfall: {details['favorable_conditions']['rainfall']}")
                            
                            st.markdown(f"**⚠️ Critical Stages:** {', '.join(details['damage_stages'])}")
                            st.markdown(f"**📊 Economic Threshold:** {details['economic_threshold']}")
                        
                        with col2:
                            st.markdown("**👁️ Key Symptoms:**")
                            for symptom in details['symptoms'][:4]:  # Show top 4 symptoms
                                st.markdown(f"• {symptom}")
                            
                            st.markdown(f"**🔍 Monitoring:** {details['monitoring']}")
                        
                        # Management strategies tabs
                        mgmt_tab1, mgmt_tab2 = st.tabs(["🌿 Organic Control", "💊 Chemical Control"])
                        ...
                        with mgmt_tab2:
                            ...
                            for i, method in enumerate(details['chemical_control'], 1):
                                st.markdown(f"{i}. {method}")
                            st.warning("⚠️ Always follow label instructions and observe waiting periods before harvest")
        
        # Crop-specific risks
        if pest_analysis['crop_specific_risks']['high_risk_stage']:
            st.error(f"🚨 HIGH ALERT: {growth_stage} stage is high-risk for {crop_type} pests!")
    
    # Irrigation Management Tab
    with tabs[3]:
        st.markdown('<div class="tab-header"><h2>💧 Irrigation Management System</h2></div>', unsafe_allow_html=True)
        ...
        irrigation_priority = "HIGH" if irrigation_recs['is_critical_stage'] else "MODERATE" if irrigation_recs['daily_water_requirement'] > 5 else "LOW"
        priority_color = "#FF4444" if irrigation_priority == "HIGH" else "#FFAA00" if irrigation_priority == "MODERATE" else "#44AA44"
        ...
        with col1:
            ...
            st.metric(
                "📅 Frequency", 
                irrigation_recs['irrigation_frequency']
            )
        
        with col2:
            st.metric(
                "🏭 Recommended Method", 
                irrigation_recs['recommended_method']
            )
            st.metric(
                "💧 Water Source", 
                irrigation_recs['water_source']
            )
        
        with col3:
            st.metric(
                "⚡ Zone Efficiency", 
                f"{irrigation_recs['zone_efficiency'] * 100:.0f}%"
            )
            st.metric(
                "🌾 Seasonal Total", 
                f"{irrigation_recs['seasonal_total']} mm"
            )
        ...
        with col1:
            ...
            factors_data = {
                ...
                'Impact': [
                    float(irrigation_recs['current_factors']['temperature_impact'].replace('%', '').replace('+', '')),
                    float(irrigation_recs['current_factors']['humidity_impact'].replace('%', '').replace('+', '')),
                    float(irrigation_recs['current_factors']['wind_impact'].replace('%', '').replace('+', '')),
                    float(irrigation_recs['current_factors']['growth_stage_impact'].replace('%', '').replace('+', ''))
                ],
                'Values': [
            ...
            colors = ['#FF7043' if x > 0 else '#4CAF50' if x < 0 else '#03A9F4' for x in impact_df['Impact']]
            
            impact_fig = go.Figure(data=[go.Bar(
                x=impact_df['Factor'],
                y=impact_df['Impact'],
                marker_color=colors,
                text=[f"{val}<br>({imp:+.1f}%)" for val, imp in zip(impact_df['Values'], impact_df['Impact'])],
                textposition='auto',
        ...
        with col2:
            ...
            if irrigation_recs['irrigation_frequency'] == 'Daily':
                ...
                irrigation_schedule = [irrigation_recs['water_per_hectare'] if i % 2 == 0 else 0 for i in range(7)]
            else:  # Every 3 days
                irrigation_schedule = [irrigation_recs['water_per_hectare'] if i % 3 == 0 else 0 for i in range(7)]
            
            schedule_colors = ['#4CAF50' if val > 0 else '#8D6E63' for val in irrigation_schedule]
            
            schedule_fig = go.Figure(data=[go.Bar(
                x=days,
                y=irrigation_schedule,
                marker_color=schedule_colors,
                text=[f"{int(val)} L" if val > 0 else "Rest" for val in irrigation_schedule],
                textposition='auto'
            )])
        ...
        if irrigation_recs['is_critical_stage']:
            st.error(f"🌸 CRITICAL STAGE ALERT: {growth_stage} is a critical growth stage for {crop_type}!")
            st.markdown("### ⚠️ Critical Growth Stage Management")
            st.info(f"Critical stages for {crop_type}: {', '.join(irrigation_recs['critical_growth_stages'])}")
        ...
        today_actions = [
            {"icon": "💧", "action": "Water Today", "time": "6-8 AM or 6-8 PM", "amount": f"{irrigation_recs['water_per_hectare']:.0f}L"},
            {"icon": "📊", "action": "Check Soil", "time": "Before watering", "amount": "2-3 inch depth"},
            {"icon": "🌡️", "action": "Monitor Weather", "time": "Check forecast", "amount": "Adjust as needed"}
        ]
        ...
        with rec_col2:
            ...
            st.info(f"**Water Efficiency:** {irrigation_recs['zone_efficiency']*100:.0f}%")
        ...
        weekly_cost = daily_cost * 7 if irrigation_recs['irrigation_frequency'] == 'Daily' else daily_cost * 3.5 if irrigation_recs['irrigation_frequency'] == 'Every 2 days' else daily_cost * 2.33
    ...
    # Zone Mapping Tab
    with tabs[4]:
        st.markdown('<div class="tab-header"><h2>🗺️ Maharashtra Zone Mapping</h2></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            try:
                ...
                st.warning(f"Google Maps unavailable, using enhanced map view. Error: {str(e)}")
                ...
                try:
                    ...
                    fig = px.scatter_mapbox(
                        ...
                        size='size',
                        hover_name='District',
                        hover_data={'lat': False, 'lon': False, 'size': False, 'Zone': True},
                    ...
                    fig2 = px.scatter(
                        df,
                        x='lon', y='lat', color='Zone', size='size',
                        hover_name='District',
                        title=f'Maharashtra Districts — {zone} zone highlighted',
                        color_discrete_map=zone_colors, height=520
                    )
                    if district in system.district_coords:
                        sel_lat, sel_lon = system.district_coords[district]
                        fig2.add_scatter(
                            x=[sel_lon], y=[sel_lat], mode='markers',
                            marker=dict(size=28, color='#F44336', symbol='star'),
                            name=f'{district} (Selected)', showlegend=False
                        )
                    fig2.update_layout(
                        margin=dict(l=0, r=0, t=48, b=0),
        ...
        zone_comparison_data = {
            ...
            'Districts': [len(districts) for districts in system.maharashtra_districts.values()],
        ...
        for i, (zone_name, zone_districts) in enumerate(system.maharashtra_districts.items()):
            with zone_cols[i]:
                zone_color = zone_colors.get(zone_name, '#666666')
                selected_indicator = "🎯 SELECTED" if zone_name == zone else "View Details"
    ...
    # Dashboard Tab
    with tabs[5]:
        st.markdown('<div class="tab-header"><h2>Farm Analytics Dashboard</h2></div>', unsafe_allow_html=True)
        ...
        with col1:
            st.markdown(f'<div style="background: linear-gradient(135deg, #2E7D32 0%, #4CAF50 100%); padding: 1rem; border-radius: 10px; text-align: center; color: white; box-shadow: 0 4px 15px rgba(46, 125, 50, 0.3);"><h4>🏠 Farm Size</h4><h2>{farm_area} Ha</h2></div>', unsafe_allow_html=True)
        
        with col2:
            st.markdown(f'<div style="background: linear-gradient(135deg, #5D4E37 0%, #8D6E63 100%); padding: 1rem; border-radius: 10px; text-align: center; color: white; box-shadow: 0 4px 15px rgba(93, 78, 55, 0.3);"><h4>🌾 Crop Type</h4><h2>{crop_type}</h2></div>', unsafe_allow_html=True)
        
        with col3:
            st.markdown(f'<div style="background: linear-gradient(135deg, #1976D2 0%, #03A9F4 100%); padding: 1rem; border-radius: 10px; text-align: center; color: white; box-shadow: 0 4px 15px rgba(25, 118, 210, 0.3);"><h4>🌱 Growth Stage</h4><h2>{growth_stage}</h2></div>', unsafe_allow_html=True)
        
        with col4:
            st.markdown(f'<div style="background: linear-gradient(135deg, #FF9800 0%, #FFA726 100%); padding: 1rem; border-radius: 10px; text-align: center; color: white; box-shadow: 0 4px 15px rgba(255, 152, 0, 0.3);"><h4>🌤️ Temperature</h4><h2>{current_weather["temperature"]}°C</h2></div>', unsafe_allow_html=True)
        
        with col5:
            location_info = f"{district}, {zone}"
            st.markdown(f'<div style="background: linear-gradient(135deg, #FF7043 0%, #F44336 100%); padding: 1rem; border-radius: 10px; text-align: center; color: white; box-shadow: 0 4px 15px rgba(255, 112, 67, 0.3);"><h4>📍 Location</h4><h2 style="font-size: 0.8rem;">{location_info}</h2></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            if 'crop_analysis' in st.session_state and st.session_state.crop_analysis is not None:
                result = st.session_state.crop_analysis
                health_color = 'linear-gradient(135deg, #4CAF50 0%, #66BB6A 100%)' if result['disease'].lower() == 'healthy' else 'linear-gradient(135deg, #F44336 0%, #FF7043 100%)'
                st.markdown(f'<div style="background: {health_color}; padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>🌿 Crop Health Status</h3><h1>{result["disease"]}</h1><p>Confidence: {result["confidence"]:.1f}%</p></div>', unsafe_allow_html=True)
            else:
                st.markdown('<div style="background: linear-gradient(135deg, #8D6E63 0%, #5D4E37 100%); padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>🌿 Crop Health Status</h3><h1>Not Analyzed</h1><p>Upload image and analyze</p></div>', unsafe_allow_html=True)
            
            # Soil Health Summary
            if 'soil_analysis' in st.session_state and st.session_state.soil_analysis is not None:
                soil = st.session_state.soil_analysis
                soil_color = 'linear-gradient(135deg, #4CAF50 0%, #66BB6A 100%)' if soil['score'] > 70 else 'linear-gradient(135deg, #FF9800 0%, #FFA726 100%)' if soil['score'] > 50 else 'linear-gradient(135deg, #F44336 0%, #FF7043 100%)'
                st.markdown(f'<div style="background: {soil_color}; padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>🧪 Soil Health</h3><h1>{soil["score"]}/100</h1><p>Status: {soil["status"]}</p></div>', unsafe_allow_html=True)
            else:
                st.markdown('<div style="background: linear-gradient(135deg, #8D6E63 0%, #5D4E37 100%); padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>🧪 Soil Health</h3><h1>Not Analyzed</h1><p>Run analysis to see results</p></div>', unsafe_allow_html=True)
        
        with col2:
            # Pest Risk Summary
            if 'pest_analysis' in st.session_state and st.session_state.pest_analysis is not None:
                pest = st.session_state.pest_analysis
                risk_color = pest['risk_level']['color']
                st.markdown(f'<div style="background: {risk_color}; padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0;"><h3>🐛 Pest Risk Level</h3><h1>{pest["risk_level"]["level"]}</h1><p>Risk Score: {pest["overall_risk"]}/100</p></div>', unsafe_allow_html=True)
            else:
                st.markdown('<div style="background: #666; padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0;"><h3>🐛 Pest Risk Level</h3><h1>Not Analyzed</h1><p>Run analysis to see results</p></div>', unsafe_allow_html=True)
            
            # Irrigation Summary
            if 'irrigation_analysis' in st.session_state and st.session_state.irrigation_analysis is not None:
                irrigation = st.session_state.irrigation_analysis
                irrigation_priority = "HIGH" if irrigation['is_critical_stage'] else "MODERATE" if irrigation['daily_water_requirement'] > 5 else "LOW"
                irrigation_color = 'linear-gradient(135deg, #F44336 0%, #FF7043 100%)' if irrigation_priority == 'HIGH' else 'linear-gradient(135deg, #FF9800 0%, #FFA726 100%)' if irrigation_priority == 'MODERATE' else 'linear-gradient(135deg, #03A9F4 0%, #1976D2 100%)'
                st.markdown(f'<div style="background: {irrigation_color}; padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>💧 Irrigation Priority</h3><h1>{irrigation_priority}</h1><p>{irrigation["daily_water_requirement"]:.1f} mm/day</p></div>', unsafe_allow_html=True)
            else:
                st.markdown('<div style="background: linear-gradient(135deg, #8D6E63 0%, #5D4E37 100%); padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>💧 Irrigation Status</h3><h1>Not Analyzed</h1><p>Run analysis to see results</p></div>', unsafe_allow_html=True)
            
            # Weather Summary
            weather_status = "Good" if 20 <= current_weather['temperature'] <= 30 and current_weather['humidity'] <= 70 else "Monitor"
            weather_color = 'linear-gradient(135deg, #4CAF50 0%, #66BB6A 100%)' if weather_status == 'Good' else 'linear-gradient(135deg, #FF9800 0%, #FFA726 100%)'
            st.markdown(f'<div style="background: {weather_color}; padding: 1.5rem; border-radius: 15px; color: white; margin: 1rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.2);"><h3>🌤️ Weather Status</h3><h1>{weather_status}</h1><p>{current_weather["description"].title()}</p></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            if 'crop_analysis' in st.session_state and st.session_state.crop_analysis is not None and st.session_state.crop_analysis['disease'].lower() != 'healthy':
                priority_actions.append(f"🌿 URGENT: Treat {st.session_state.crop_analysis['disease']}")
            
            if 'pest_analysis' in st.session_state and st.session_state.pest_analysis is not None and st.session_state.pest_analysis['overall_risk'] > 70:
                priority_actions.append("🐛 HIGH: Implement pest control measures")
            
            if 'soil_analysis' in st.session_state and st.session_state.soil_analysis is not None and st.session_state.soil_analysis['score'] < 50:
                priority_actions.append("🧪 CRITICAL: Improve soil health immediately")
            
            if 'irrigation_analysis' in st.session_state and st.session_state.irrigation_analysis is not None and st.session_state.irrigation_analysis['is_critical_stage']:
                priority_actions.append(f"💧 URGENT: Critical irrigation needed - {growth_stage} stage")
